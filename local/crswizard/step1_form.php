...
function step1_form() {
    ...
    $formstep1 .= $OUTPUT->single_button(
            new moodle_url('', array('step' => 2, 'wizardcase' => 1)), 'Un élément pédagogique dans lequel j\'enseigne', 'post'
    );
    $formstep1 .= '</div><div style="margin:5px;">';
    $formstep1 .= $OUTPUT->single_button(
            new moodle_url('', array('step' => 2, 'wizardcase' => 2)), 'Un autre élément pédagogique de l\'offre de formation', 'post'
    );
    $formstep1 .= '</div><div style="margin:5px;">';
    $formstep1 .= $OUTPUT->single_button(
            new moodle_url('/local/crswizard/index.php', array('stepin' => 1, 'stepgo_2' => 2, 'wizardcase' => 3)), get_string('anotherneed', 'local_crswizard'), 'post'
    );
    $formstep1 .= '</div></div>';
    return $formstep1;
}
